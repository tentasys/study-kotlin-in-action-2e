...
fun main() {
    ...
    // 로또 번호 섞기 및 출력
    val numberList: List<Int> = (1..45).toList()
    val boughtLottos: MutableList<List<Int>> = mutableListOf()
    for (i in 1..buyCount) {
        val lottos: List<Int> = numberList.shuffled().take(LOTTO_COUNT).sorted()
        println(lottos)
    ...
    val resultPriceRate = (resultAmount.toDouble() / purchaseAmount.toDouble()).toBigDecimal().setScale(2, RoundingMode.DOWN)
